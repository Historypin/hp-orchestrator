...
public class ListRecords extends HarvesterVerb {
	 ...
	 */
	public ListRecords(String baseURL, String from, String until, String set,
			String metadataPrefix, String authorizationString) throws IOException,
			ParserConfigurationException, SAXException, TransformerException {
		super(getRequestURL(baseURL, from, until, set, metadataPrefix), authorizationString);
	}
	 ...
	 * @throws TransformerException
	 */
	public ListRecords(String baseURL, String resumptionToken, String authorizationString)
			throws IOException, ParserConfigurationException, SAXException,
			TransformerException {
		super(getRequestURL(baseURL, resumptionToken), authorizationString);
	}
	 ...
	 */
	public static String getRequestURL(String baseURL, String from,
		...
		if (from != null){
            try {
                requestURL.append("&from=").append(URLEncoder.encode(from, "utf-8"));
		...
		if (until != null){
            try {
                requestURL.append("&until=").append(URLEncoder.encode(until, "utf-8"));
            } catch (UnsupportedEncodingException e) {
		...
		if (set != null){
    		try {
                requestURL.append("&set=").append(URLEncoder.encode(set, "utf-8"));
            } catch (UnsupportedEncodingException e) {
